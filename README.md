...
TMDB API  →  01_extração_api.py  →  filmes_populares.csv       (1.000 registros brutos)
                    ↓ limpeza
                                     filmes_populares_clean.csv  (978 registros)
                    ↓
...
| Após limpeza | 978 |
| Descartados | 22 (20 sem sinopse + 2 duplicados) |
| Período | 1902 – 2025 |
...
entrada do modelo —, **deduplicação por `id` do TMDB** (a paginação da API devolve o mesmo filme em
páginas diferentes), conversão de `release_date` para datetime com descarte de datas inválidas, e
derivação das colunas `year` e `month`.

Restam 5 títulos repetidos no catálogo, mas com `id` distinto: são remakes legítimos —
`Os Suspeitos` (1995 e 2013), `Como Treinar o Seu Dragão` (2010 e 2025), `12 Homens e uma Sentença`
(1957 e 1997), entre outros. O app exibe o ano ao lado do título para desambiguar.
...
tfidf_matrix = tfidf.fit_transform(df['overview'])   # (978, 10290)
cosine_sim = cosine_similarity(tfidf_matrix)
```

| | |
|---|---|
| Matriz TF-IDF | 978 × 10.290 |
| Vocabulário | 10.290 termos |
| Densidade | 0,31% (matriz esparsa) |
| Stopwords PT-BR | 207 termos (NLTK) |
| Matriz de similaridade | 978 × 978 · ~7,7 MB em memória |
...
(93), consistente com a temporada de férias do hemisfério norte, mas o pico verdadeiro é o de fim de
ano.
...
**Gênero também quase não separa nota.** Da Música (7,94) ao Terror (7,80), a amplitude é de
**0,14 ponto** entre 19 gêneros.

**A explicação para os dois achados anteriores está no dataset, não no cinema.** Como a coleta usa
`top_rated`, todos os 978 filmes já passaram por um filtro de qualidade — a variável resposta está
...
**Volume cresce, qualidade não segue.** 380 filmes (39% do catálogo) são de 2013 em diante, com pico
em 2019 (50 filmes). 2022 foi um dos anos de maior volume e ficou com nota média de 7,83, abaixo da
média geral.
...
| Média | 0,125 |
| Mediana | 0,114 |
| Máximo do dataset | 0,461 |
| Filmes cujo melhor match fica abaixo de 0,15 | **766 de 978 (78%)** |
...
## Limitações e correções aplicadas
...
- **Não escala como está.** A matriz de similaridade é O(n²): 978 filmes ocupam 7,7 MB, mas 50 mil
...
Cinco defeitos encontrados na revisão do código foram corrigidos:

- **Registros duplicados no dataset.** `Guerreiras do K-Pop` e `Marcados Pelo Sangue` apareciam duas
  vezes com o **mesmo `id`** do TMDB, efeito da paginação da API. A consequência era visível no
  produto: o par mais similar de todo o dataset era um filme com ele mesmo (cosseno 1,000) e o app
  devolvia o próprio filme selecionado como primeira recomendação. Resolvido com
  `drop_duplicates(subset='id')` na limpeza — a similaridade máxima do dataset caiu de 1,000 para
  0,461, que é o par legítimo `Meus Caros Amigos` (1975) e `Quinteto Irreverente` (1982).
- **Remakes colidiam por título.** `app.py` resolvia a seleção com
  `df[df['title'] == selected].index[0]`, então escolher `Os Suspeitos` (2013) devolvia as
  recomendações de `Os Suspeitos` (1995). A seleção passou a ser pela posição da linha, com o ano
  exibido no seletor.
- **Fallback de imagem apontava para serviço fora do ar.** `get_poster()` devolvia uma URL de
  `via.placeholder.com`, que não responde mais — a falha virava imagem quebrada. Agora devolve `None`
  e o app desenha um placeholder local.
- **`except` silencioso.** O `try/except` de `get_poster()` engolia qualquer exceção sem log,
  deixando erro de chave de API e falha de rede indistinguíveis. Agora captura `RequestException`,
  usa `timeout=5` e reporta o motivo com `st.warning`.
- **`nltk.download('stopwords')` ia à rede a cada start.** Passou a ser condicional a `LookupError`,
  em `app.py` e em `03_modelo.py`.

---

## Próximos passos

1. **Enriquecer o vetor de conteúdo** concatenando gênero, diretor e elenco à sinopse antes do
  TF-IDF. É o caminho mais direto para tirar a similaridade média da faixa de 0,12, já que ataca
  justamente a escassez de vocabulário.
2. **Trocar TF-IDF por embeddings de sentença** (ex.: `sentence-transformers`), que capturam
...
3. **Filtragem colaborativa** sobre avaliações de usuários, para um sistema híbrido conteúdo +
  comportamento.
4. **Avaliação com ground truth**: usar as coleções e os `similar` do próprio TMDB como referência
  para medir precision@5 e ter um número comparável entre versões.
5. **Ampliar a amostragem** para além do `top_rated`, incluindo `/movie/popular` ou `/discover`, para
  recuperar a variação de nota que hoje o dataset não tem.
...
├── filmes_populares_clean.csv   # dados tratados (978)
└── requirements.txt
```
